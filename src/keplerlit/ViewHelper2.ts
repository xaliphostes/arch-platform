...
class ViewHelper2 extends Object3D {
    ...
    superParent: Object3D

    constructor(
        camera: PerspectiveCamera | OrthographicCamera,
        renderer: WebGLRenderer,
        placement: DomPlacement = "bottom-left",
        size: number = 128
    ) {
        super()
        this.superParent = this as Object3D
        ...
        this.domContainer = getDomContainer(placement, size, parent);
    ...
    onPointerDown(e: PointerEvent) {
        const drag = (e: PointerEvent) => {
            ...
            this.superParent.rotation.x = clamp(
                rotationStart.x + mouseAngle.y,
                Math.PI / -2 + 0.001,
                Math.PI / 2 - 0.001
            );
            this.superParent.rotation.y = rotationStart.y + mouseAngle.x;
            this.superParent.updateMatrixWorld();

            q1.copy(this.superParent.quaternion).invert();
        ...
        const rotationStart = euler.copy(this.superParent.rotation);
    ...
    render() {
        ...
        // Compute relative to the canvas, not the page
        const canvasRect = this.domElement.getBoundingClientRect();
        const x = this.domRect.left - canvasRect.left;
        const y = canvasRect.bottom - this.domRect.bottom; // or: (this.domRect.top - canvasRect.top) from top
    ...
    updateOrientation(fromCamera: boolean = true) {
        if (fromCamera) {
            this.superParent.quaternion.copy(this.camera.quaternion).invert();
            this.superParent.updateMatrixWorld();
        }
...
function getDomContainer(
    ...
    const ID = 'view-helper-container';
    ...
    // chercher d'abord DANS le parent (évite collisions globales)
    let div = parent.querySelector<HTMLDivElement>(`#${ID}`);
    if (!div) {
        div = document.createElement('div');
        div.id = ID;
        parent.appendChild(div); // <-- append ici, une seule fois
    }
    ...
    style.zIndex = '10001';
    style.pointerEvents = 'auto';          // ne bloque pas l’orbite
    //style.background = 'rgba(255,255,255,0.8)'; // utile pour debug
    ...
    // horizontal
    if (x === 'left') style.left = '0';
    else if (x === 'right') style.right = '0';
    else { style.left = '50%'; style.transform += 'translateX(-50%)'; }

    // vertical
    if (y === 'top') style.top = '0';
    else if (y === 'bottom') style.bottom = '0';
